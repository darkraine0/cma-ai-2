...
- Use analyze_community_changes whenever the user wants to see, list, show, browse, analyze, or review plans for a community. It performs a live web search for current floor plans and quick move-ins (spec homes) from all builders in that US community, then shows a "Show Plans" button that opens an in-chat modal. Examples: "show plans for Elevon" → analyze_community_changes(community_name_or_query="Elevon"); "what plans are available in Cambridge Crossing" → analyze_community_changes(community_name_or_query="Cambridge Crossing").
